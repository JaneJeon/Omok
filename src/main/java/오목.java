...
public class 오목 extends JFrame {
	...
	private void drawPieces(Graphics g) {
		...
		if (show >= 1) {
			...
			if (!ifWon && show == pieces.size()) {
				g.fillOval(offset + square * pieces.get(show - 1).x - (int) (pieceSize * lastPieceScale), offset + 
					square * pieces.get(show - 1).y - (int) (pieceSize * lastPieceScale), (int) (pieceSize * 2 *
					lastPieceScale), (int) (pieceSize * 2 * lastPieceScale));
			} else {
				g.fillOval(offset + square * pieces.get(show - 1).x - pieceSize, offset + square * 
					pieces.get(show - 1).y - pieceSize, pieceSize * 2, pieceSize * 2);
			}
			if ((show - 1) % 2 == 0) {
				g.setColor(Color.white);
			} else {
					...
					for (int i = 0; i < pieces.size(); i++) {
						...
						if ((p.x - px) * (p.x - px) + (p.y - py) * (p.y - py) < 1) {
							...
							if (i != pieces.size() - 1) {
								...
								g.fillOval(offset + square * px - (int) (pieceSize * lastPieceScale), offset + 
										square * py - (int) (pieceSize * lastPieceScale), 
									(int) (pieceSize * 2 * lastPieceScale), (int) (pieceSize * 2 * lastPieceScale));
							}
							return;
						}
					}
				...
				if (AIMode || legalMove(pt)) { // legitimate move by the user
					...
					if (!online) { // when offline, check for win
						if (won()) {
							...
							playSound(-5);
							if (pieces.size() % 2 == 0) {
								JOptionPane.showMessageDialog(this, whiteWinString,
									endString, JOptionPane.INFORMATION_MESSAGE);
							} else {
								JOptionPane.showMessageDialog(this, blackWinString,
									endString, JOptionPane.INFORMATION_MESSAGE);
							}
							repaint();
						} else {
					...
					JOptionPane.showMessageDialog(this, illegalString, errorString, JOptionPane.ERROR_MESSAGE);
				}
			}
		}
	}
	...
	public void callback(Point p) {
		...
		if (won()) {
			...
			JOptionPane.showMessageDialog(this, cpuString, endString,
		...
		if ((!ifWon || TEST) && pieces.size() > 0) {
			...
			if ((pieces.size() % 2 == 1 && bUndo < 3) || (pieces.size() % 2 == 0 && wUndo < 3)) {
					...
					if (!AIMode) {
						...
						if (!AIblack || pieces.size() > 1) {
							...
							if (undoErrorCount % 2 == 0) {
								playSound(-1);
								JOptionPane.showMessageDialog(this, backString, errorString,
				...
				if (!AIMode || undoErrorCount % 2 == 0) {
					playSound(-1);
					JOptionPane.showMessageDialog(this, backString, errorString, JOptionPane.ERROR_MESSAGE);
				}
				undoErrorCount++;
			}
			repaint();
	...
	private void load() {
		...
		FileNameExtensionFilter filter = new FileNameExtensionFilter("TEXT FILES", "txt", "text");
		fileChooser.setFileFilter(filter);
		if (fileChooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION && !AIMode && !online) {
			File file = fileChooser.getSelectedFile();
			try {
				...
				System.out.println("set34: " + set34);
				ifWon = won();
				repaint();
			} catch (IOException e) {
				playSound(-1);
	...
	public void checkWin() {
		...
		if (won()) {
			...
			if (pieces.size() % 2 == 0) {
				playSound(-5);
				JOptionPane.showMessageDialog(this, whiteWinString, endString, JOptionPane.INFORMATION_MESSAGE);
			} else {
				playSound(-5);
				JOptionPane.showMessageDialog(this, blackWinString, endString, JOptionPane.INFORMATION_MESSAGE);
			}
		}
	}
	...
	private Jack newAI(int difficulty) {
		if (difficulty != 4) {
			...
			return new Jack(false).defenseWeight(testParamsDouble[0]).threshold(testParamsDouble[1])
				...
				.branchLimit(testParamsInt[2]).depth(testParamsInt[3]).board(this);
		}
	}
}